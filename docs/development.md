...
`bun run check` runs Biome and TypeScript. `bun run test` runs unit, component, integration, contract, and security tests. `bun run package` compiles `src/cli.ts` to `builds/volare`. See [Testing](testing.md) for the current test commands and layer ownership.
Hook installation is explicit so the published CLI package has no install-time side effects.

## PR and release automation

Pull requests and pushes to `main` run the CI workflow in `.github/workflows/ci.yml`. The workflow splits validation into static, unit, component, integration, contract, security, package-smoke, and real Codex E2E jobs. The Codex E2E lane installs the pinned Codex CLI version declared in the workflow, records it, and runs `bun run test:e2e:codex`; the test configures a temporary Codex home using the branch's `scripts/config-codex.ts` logic before launching Codex.

Releases publish `@lachimere/volare` through `.github/workflows/release.yml`, not from a developer machine. Publish a GitHub Release whose tag matches the package version, for example `vX.Y.Z`; the workflow verifies the tag, reruns deterministic validation, smoke-tests the packed CLI with `bunx`, and runs real Codex E2E with the Codex CLI version recorded in logs before publishing. Release validation runs before the npm publish job and does not hold npm provenance/OIDC permissions. Configure npm trusted publishing for the `npm` environment before workflow-driven publishing. If npm OIDC cannot create a brand-new package, a maintainer may need a one-time bootstrap publish; after the package exists, subsequent releases should go through the workflow. The workflow also supports a manual dry run through `workflow_dispatch` and skips npm publish when the exact package version already exists. After publishing, `bunx @lachimere/volare update` refreshes Bun's bunx cache for local users.
