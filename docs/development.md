...
Pull requests and pushes to `main` run the CI workflow in `.github/workflows/ci.yml`. The workflow splits validation into static, unit, component, integration, contract, security, and package-smoke jobs. Real Codex E2E stays outside pull request gates, but runs on `main`/manual CI and during release validation.
