...
`bun run check` runs Biome and TypeScript. `bun run test` runs unit, component, target integration, contract, and security tests. `bun run package` compiles `src/cli.ts` to `builds/volare`. See [Testing](testing.md) for the current test commands and layered migration contract.
