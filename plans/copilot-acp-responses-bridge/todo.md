...
- [x] Add core interfaces and canonical types.
  ...
  - **Expected evidence**: `bun run check` passed. Core files: `src/core/types.ts`, `src/core/errors.ts`; `rg 'interface [A-Za-z]+\b' src tests scripts` shows all TypeScript interface declarations use the explicit `I` prefix.
...
- [x] Implement OpenAI Responses stored response encoding.
  - **Acceptance criteria**: `INorthboundAdapter.encodeStoredResponse()` encodes `ITurnRecord` plus accumulated canonical events for terminal and non-terminal turns.
...
- [x] Implement `IStateStore`.
  ...
  - **Expected evidence**: `src/state/sqlite-store.ts` implements `IStateStore`; unit tests cover workspace get-or-create, queued turn creation, compare-and-set turn updates, backend reserve/activate/status updates, and client ref bind/lookup.
...
- [x] Implement `ISessionManager`.
  - **Acceptance criteria**: It owns core turn start/cancel flows, backend session creation and resumption including `IAgentBackend.resumeSession()` calls, backend session continuity, terminal-event guarantees, adapter-independent orchestration, and workspace re-canonicalization before backend session resume. `ISessionManager.startTurn()` returns `IResolvedTurn` containing turn/thread/session/request and wraps `IAgentBackend.send()` in `try/finally`, expects the backend to emit a terminal event when it can, and synthesizes exactly one terminal event if the backend throws, exits early, times out, or omits a terminal event. Before every send/resume/cancel operation, it validates `session.workspaceId === request.workspaceId` and `session.threadId === request.threadId`. If the persisted workspace path is missing or resolves differently during resume, it fails with `workspace_changed`.
...
- [x] Implement `IApprovalProvider` persistence and atomic resolution.
  - **Acceptance criteria**: Approval status update and `permission.resolved` journal append commit atomically through `IStateStore.resolveApprovalWithJournal()` for allow, deny, timeout, and cancellation paths. If the journal append cannot commit, the approval remains pending or the turn fails closed with `turn.failed`; never continue with an unjournaled decision.
...
- [x] Implement `IApprovalProvider.awaitDecision()` AbortSignal cancellation.
...
- [x] Implement approval API and backend decision delivery.
  - **Acceptance criteria**: Behavior matches the Phase 0 decision and design Open Question #7 resolution, and does not claim approval enforcement without backend support. If HTTP approval UI is not in MVP, this means internal `IApprovalProvider` API plus backend integration only.
...
- [x] Implement approval timeout watchdog and escalation.
  - **Acceptance criteria**: After timeout auto-deny is journaled, `ISessionManager` waits up to `cancel_timeout_ms` for a backend terminal event; if none arrives, it atomically checks the turn is non-terminal, calls `IAgentBackend.cancel(session, { forceAfterTimeout: true })`, synthesizes `turn.interrupted` with reason `approval_timeout_exceeded`, and marks the backend session abandoned after forced disposal if needed.
...
- [x] Implement the Responses cancel endpoint route.
  - **Acceptance criteria**: `POST /openai/v1/responses/:id/cancel` requires auth, resolves external response ID to turn ID, calls `ISessionManager.cancelTurn()`, and returns appropriate success/error responses for authenticated cancel, unauthenticated rejection, missing response ID, already-terminal turn, and concurrent cancel.
...
- [x] Implement force-dispose process cleanup.
  - **Acceptance criteria**: Timeout escalation calls backend cleanup, marks sessions disposed/abandoned from `ISessionManager`, and does not leave reusable abandoned sessions. Process-backed backends escalate disposal with SIGTERM first, then SIGKILL if the process remains alive after the shutdown budget.
