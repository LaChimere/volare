...
updated_at: "2026-05-05T20:44:00+08:00"
...
| User-visible behavior: Users can diagnose Volare-owned Codex config drift without exposing secrets. | `volare config codex doctor` is implemented in `src/cli.ts`; tests assert safe issue output and no secret-like values are printed. | met |
...
| Validation: Run repository check and full test suite before completion. | `bun run check` and `bun run test` passed. | met |
